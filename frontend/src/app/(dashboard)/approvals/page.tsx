...
import { useApproveListing, useFetchUnApprovedListings } from "@/hooks/useFetchBackend";
...
// console.log({state, index, id})
  ...
  return (
    <div className="flex-1 flex flex-col gap-4">
      <h1 className="text-xl md:text-2xl capitalize font-bold">Unapproved Listings</h1>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {isLoading ? Array.from({ length: 8 }).map((_, _key) => (
          <div key={_key + Math.random() * 50} className="flex flex-col gap-2 w-full">
        ...
        )) : listings?.length === 0 ? <p>No listings to approve</p> :
          listings?.map((listing: any, _key: number) => {
            ...
            return <div className="flex flex-col gap-2" key={_key}>
              <Link href={`/listing/${lt.id}`} className="flex flex-col gap-2 w-full group">
                ...
                <div className="flex flex-col">
                  <h1 className="text-sm font-medium text-muted-foreground">{shortenAddress(lt.owner)}</h1>
                  <p className="text-sm line-clamp-1">{lt.description}</p>
                </div>
              </Link>

              <div className="flex gap-2">
                <Button className="w-full" disabled={isApproving || isApproved} variant="secondary" onClick={e => {
                  e.stopPropagation();

                  approveListing(lt.region[0], Number(listing[2].toString()), lt.id);
