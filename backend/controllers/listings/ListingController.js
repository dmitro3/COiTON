...
const {
  SendListingTransaction,
} = require("../../helpers/ContractInterraction");
...
exports.createListing = async (req, res) => {
  try {
    ...
    const indices = await MarketIndices.create({
      estateId: newListing.id,
      indexValue: data.price,
    });
    const tx = await SendListingTransaction(newListing.id, data);
    return ResponseMessage(res, true, 200, "Listing created successfully", {
      newListing,
      tx,
      indices,
    });
...
exports.updateMarketIndice = async (req, res) => {
  try {
    ...
    if (find) {
      ...
      return ResponseMessage(res, true, 200, "Indice created successfully", {
        ...indices,
      });
    }
